...
-- Inserta en 'usuario' y luego, dependiendo del 'p_rol', en la
-- tabla de especialización correspondiente (lector, bibliotecario
-- o administrador) dentro de UNA transacción.
-- =========================================================
...
CREATE PROCEDURE sp_registrar_usuario (
    ...
    IN  p_rol                  VARCHAR(20), -- 'lector', 'bibliotecario', 'administrador'
    
    -- Datos específicos de Lector (opcionales para otros roles)
...
BEGIN
    ...
    DECLARE v_codigo_generado VARCHAR(10);
    ...
    IF p_password_hash IS NULL OR p_password_hash = '' THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'No se recibió la contraseña cifrada.';
    END IF;

    IF p_nombre IS NULL OR TRIM(p_nombre) = '' OR p_apellido IS NULL OR TRIM(p_apellido) = '' THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Debe ingresar nombre y apellido.';
    END IF;
    SET p_nombre = TRIM(p_nombre);
    SET p_apellido = TRIM(p_apellido);
    SET p_email = TRIM(p_email);

    -- Validación para rol Lector
    IF p_rol = 'lector' THEN
        IF p_tipo_lector NOT IN ('estudiante', 'docente', 'personal_administrativo', 'externo') THEN
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'tipo_lector inválido. Use: estudiante, docente, personal_administrativo o externo.';
        END IF;
    ...
    IF v_id_estado_activo IS NULL THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'No se encontró el estado "activo" en el catálogo (configuración corrupta).';
    END IF;
    ...
    BEGIN
        ...
        START TRANSACTION;
            ...
            VALUES (p_nombre, p_apellido, p_email, p_password_hash, p_rol, p_documento_identidad, p_telefono, v_id_estado_activo);
            ...
            ELSEIF p_rol = 'bibliotecario' THEN
                ...
                VALUES (p_id_usuario, v_codigo_generado);

            ELSEIF p_rol = 'administrador' THEN
                SET v_codigo_generado = CONCAT('A', LPAD(p_id_usuario, 3, '0'));
